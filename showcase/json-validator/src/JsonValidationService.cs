...
namespace JsonValidator
{
    public class JsonValidationService
    {
        ...
        public static JsonValidationService CreateFromEmbeddedResource()
        {
            try
            {
                // Load the EdFi.DataStandard52.ApiSchema assembly from the same directory as this executable
                var assemblyDir =
                    Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location)
                    ?? throw new InvalidOperationException(
                        "Could not determine the assembly directory for loading embedded resources."
                    );
                var assembly = Assembly.LoadFrom(
                    Path.Combine(assemblyDir, "EdFi.DataStandard52.ApiSchema.dll")
                );
                ...
                {
                    throw new InvalidOperationException(
                        "Failed to parse API schema JSON from embedded resource"
                    );
                }
            ...
            catch (Exception ex)
            {
                ...
                {
                    throw new InvalidOperationException(
                        $"Could not load API schema from embedded resource ({ex.Message}) and fallback file not found at {apiSchemaPath}"
                    );
                }
        ...
        public string ExtractResourceTypeFromPath(string filePath)
        {
            ...
            if (parentFullPath == "/" || parentFullPath == "\\" || parentDirectory.Length <= 2) // Likely drive letters or very short names
            {
                throw new ArgumentException($"Cannot extract resource type from path: {filePath}");
            }
        ...
        public JsonSchema? GetSchemaForResourceType(string resourceType)
        {
            ...
            {
                var jsonPath =
                    $"$.projectSchema.resourceSchemas.{resourceType}.jsonSchemaForInsert";
            ...
            try
            {
                ...
                if (schema == null)
                {
                    return new ValidationResult
                    {
                        IsValid = false,
                        ErrorMessage =
                            $"No schema found for resource type '{resourceType}'. Please verify the file path contains a valid Ed-Fi resource type directory.",
                    };
                }
