...
**Reverse lookup support**: ReferenceEdges can be efficiently queried to find which documents reference a specific target. By querying ReferenceEdges with `AliasLookupId` and `ReferentialPartitionKey`, you can find all parent documents that reference a given document. While this query may span multiple partitions of ReferenceEdges (since it partitions on ParentDocumentPartitionKey), the index on (AliasLookupId, ReferentialPartitionKey, ParentDocumentId) enables efficient seeks directly to the target alias. This avoids the need for joins across Documents, Aliases, and References tables that would be required in the baseline design.
